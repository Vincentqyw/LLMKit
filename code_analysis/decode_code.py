...
def analyze_source_code(
    ...
    for index, fp in enumerate(file_manifest):
        ...
        prefix = (
            "Next, please analyze the following project files one by one"
            if index == 0
            else ""
        )
        i_say = (
            prefix
            + f"Please give an overview of the following program file with the file name {os.path.relpath(fp, project_folder)} and the file code as ```{file_content}```"
        )
        i_say_show_user = (
            prefix
            + f"[{index}/{len(file_manifest)}] Please give an overview of the following program file: {os.path.abspath(fp)}"
        )
...
def analyze_project_itself(
    ...
    for index, fp in enumerate(file_manifest):
        ...
        prefix = (
            "Next, please analyze your own program structure. Don't be nervous,"
            if index == 0
            else ""
        )
        i_say = (
            prefix
            + f"Please provide an overview of the following program file with the file name {fp} and the file code as ```{file_content}```"
        )
        i_say_show_user = (
            prefix
            + f"[{index}/{len(file_manifest)}] Please provide an overview of the following program file: {os.path.abspath(fp)}"
        )
        chatbot.append((i_say_show_user, "[Local Message] waiting gpt response."))
        yield chatbot, history, "Normal"
...
def analyze_python_project(
    ...
    if os.path.exists(txt):
        ...
        report_exception(
            ...
            b=f"Local project not found or inaccessible: {txt}",
        )
        yield chatbot, history, "Normal"
        return
    ...
    if len(file_manifest) == 0:
        report_exception(
            ...
            history,
            a=f"Analyzing project: {txt}",
            b=f"No Python files found: {txt}",
        )
        yield chatbot, history, "Normal"
        return
    yield from analyze_source_code(
        file_manifest,
        project_folder,
        top_p,
        temperature,
...
def analyze_c_project_header_files(
    ...
    if len(file_manifest) == 0:
        report_exception(
            ...
            history,
            a=f"Analyzing project: {txt}",
            b=f"No .h header files found: {txt}",
        )
        yield chatbot, history, "Normal"
        return
    yield from analyze_source_code(
